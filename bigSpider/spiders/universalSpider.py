...
class UniversalSpider(scrapy.Spider, BaseFunctions):
    ...
    def __init__(self, conf=None, crawl_no=None, name=None, **kwargs):
        super(UniversalSpider, self).__init__(name, **kwargs)
    ...
    def start_requests(self):
        if 'start_url' in self.conf:
            yield Request(self.conf.get('start_url'), callback=self.parse_start_url)
    ...
    def parse_single_page(self, response, follow):
        ...
        if 'columns' in follow:
            ...
            for column in follow.get('columns'):
                ...
                # if column.get('keep_original_style', False):
                #     item_content = item_selector.extract_first()
                # else:
                #     item_content = item_selector.xpath('text()').extract_first()
                item_content = item_selector.extract_first()
    ...
    def parse_disordered_list(self, response, follow):
        ...
        if 'list' in follow:
            for single_pattern in follow['list']:
                xpath = single_pattern.get('selector', '')
